...
#%% Import data
cazyfile = 'CAZydb/cazy_data/ssd/biblio/cazy_data/cazy_data.txt'
gzipped_file = cazyfile.split('.')[0] + '.gz'

print("The file may be in compressed format due to git limits, checking for text and gziped version")
if not os.path.isfile(cazyfile):
    if os.path.isfile(gzipped_file):
        print(f"{cazyfile} not found, extracting from {gzipped_file}...")
        with gzip.open(gzipped_file, 'rb') as f_in:
            with open(cazyfile, 'wb') as f_out:
                shutil.copyfileobj(f_in, f_out)
        print("Extraction complete.")
    else:
        raise FileNotFoundError(f"Neither {cazyfile} nor {gzipped_file} found.")
else:
    print(f"{cazyfile} found.")
